...
/* Variables globales */
uint8_t HR_byte1, HR_byte2, Temp_byte1, Temp_byte2; //Guardan los byte con información de la Temp y HR
...
void DHT_GetData (DHT_DataTypedef *DHT_Data)
{
	...
	if (Presence == 1){
		...
		if (SUM == (HR_byte1+HR_byte2+Temp_byte1+Temp_byte2)) //Comprobación de envío correcto
		{
			#if defined(TYPE_DHT11)
