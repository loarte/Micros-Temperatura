...
//Nos devuelve TRUE cuando se presiona el boton tras el periodo de debounce
int debouncer(volatile int* button_int, GPIO_TypeDef* GPIO_port, uint16_t GPIO_number);

// Se encienden los LEDs correspondientes a la temperatura seleccionada y apaga los demas */
void encenderled(int i, PINES PINES_LED[]);

//INTERRUPCIONES

void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) // Cuando se pulsa el boton azul de la placa (User Button PA0), se genera la interrupción
{
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) // Genera una interrupción cada 5 segundos
{
  ...
  //Obtencion de datos del sensor (observar la librería "DHT.h")
...
//Asignación de variables
	T_diff = T_max - T_min;
	float salto_temp = T_diff / NUMERO_LED; //Empleado en el cambio en los LEDs iluminados
	int total_led = 0; //numero total de LEDs que se tienen que encender
...
//Variables locales
  ...
  //Habilitamos el ADC (Analogic Digital Conversor)
  HAL_ADC_Start(&hadc1);
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	 ...
	 if(debouncer(&Seleccion_estado, GPIOA, GPIO_PIN_0)) //Estado de seleccion de temperatura de control
	 {
		 lcd_clear();
		 lcd_put_cur(0,0);
		 ...
		 while(!debouncer(&Seleccion_estado,GPIOA, GPIO_PIN_0))
		 {
			 ...
			 //Asignacion de la temperatura de control
			 Temp_control = (float)Lec_Temp_Control / 255 * T_diff + 15; //pendiente de interpolación = 25/255

			 //Se muestra en pantalla la temperatura escogida. Para evitar el parpadeo y reducir la frecuencia de la pantalla se añade el bucle
			 if (!(i++ % 20000))
			 {
		 ...
		 //Desabilitacion del ADC
		 HAL_ADC_Stop(&hadc1);

	 }

	 else //estado para ejecutar el programa con una temperatura de control dada
	 {
		 //se muestran los valores obtenidos en el LCD (observar la libreria "i2c-lcd.h")
		 if (!(j++ % 20000))
		 {
			 Display_HR(Humedad);
			 Display_Temp(Temperatura , 0);
			 j = 1;
		 }

		 //Control de la barra LED. Truncamos el numero de LED dependiendo del salto térmico
		 total_led = (int)((Temperatura/10 - T_min) / salto_temp);
